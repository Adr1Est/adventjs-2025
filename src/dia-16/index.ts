...
function packGifts(gifts: Gifts, maxWeight: MaxWeight): Result {
  ...
  let trineos: number = 0;
  let trineoActual: number = 0;

  for(let i = 0; i < gifts.length; i++){
    if(trineoActual + gifts[i] <= maxWeight){
      trineoActual += gifts[i];
    }else{
      trineos++;
      trineoActual = gifts[i];
    }
  }
  if(trineoActual > 0) trineos++;
  return trineos;
}


export const packGiftsTest = () => {
  console.log(packGifts([2, 3, 4, 1], 5));
  // 2 trineos
  // Trineo 1: 2 + 3 = 5
  // Trineo 2: 4 + 1 = 5

  console.log(packGifts([2, 4, 3, 1], 5));
  // 3 trineos

  console.log(packGifts([5, 8], 10));
  // 2 trineos
