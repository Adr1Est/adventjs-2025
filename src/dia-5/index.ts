...
function timeUntilTakeOff(
  ...
  const elfDecoder = (elfDateTime: ElfDateTime) => {
    const [elf, _] = elfDateTime.split(" ");
    const [date, time] = elf.split("@");
    const [year, month, day] = date.split("*").map(Number);
    const [hours, minutes, seconds] = time.split("|").map(Number);
    return Date.UTC(year, month - 1, day, hours, minutes, seconds);
  }
  
  return ((elfDecoder(takeOffTime) - elfDecoder(fromTime))/1000);
}

export const timeUntilTakeOffTest = () => {
